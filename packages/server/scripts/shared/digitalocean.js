...
const updateDockerCredentials = async ($$, containerRegistryId) => {
  console.log(
    `Setting docker credentials for registry ${containerRegistryId}...`,
  );
...
exports.pushDockerImageToContainerRegistry = async (
  $$,
  containerRegistryId,
  dockerImageName,
) => {
...
const getPostgresClusters = async () => {
  ...
  return JSON.parse(body).databases.filter(
    (a) => a.connection.protocol === 'postgresql',
  );
};
...
const doesDatabaseExist = async (clusterId, dbName) => {
  try {
    console.log(
      `Checking if cluster ${clusterId} contains db named ${dbName}...`,
    );
...
const doesDatabaseUserExist = async (clusterId, userName) => {
  try {
    console.log(
      `Checking if cluster ${clusterId} contains db user named ${userName}...`,
    );
...
const grantUserFullAccessToDb = async (clusterId, userName, dbName) => {
  ...
  if (!dbret) {
    throw new Error(
      `Database ${dbName} does not exist in cluster ${clusterId}`,
    );
  }
...
exports.createDatabase = async (dbName, username) => {
  ...
  if (dbName.includes('-') || username.includes('-')) {
    console.error(
      'Database name and/or username cannot contain hyphen (-). Use underscore (_) instead.',
    );
    return;
  }
  ...
  try {
    ...
    if (!pgClusters.length) {
      console.error(
        'No postgres clusters found, please create one first via your DigitalOcean control panel.',
      );
      return;
    } else {
      ...
      if (await doesDatabaseExist(selectedCluster.id, dbName)) {
        ...
        console.log(
          `Creating database ${dbName} in cluster ${selectedCluster.id}...`,
        );
      ...
      const existingUser = await doesDatabaseUserExist(
        selectedCluster.id,
        username,
      );
      if (existingUser) {
        console.log(
          `Database user ${username} already exists, skipping create...`,
        );
        password = existingUser.user.password;
      } else {
        console.log(
          `Creating database user ${username} in cluster ${selectedCluster.id}...`,
        );

        const { body } = await callApi(
          `databases/${selectedCluster.id}/users`,
          {
            method: 'POST',
            json: {
              name: username,
