...
(async () => {
  try {
    ...
    const fs = require('fs');
    const { get$$, $$, execCommands, rootFolder, logInfoBlock } =
      await require('./shared/bootstrap')('development');
          ...
          await Promise.all([
            ...
            (async () => {
              ...
              const { local } = JSON.parse(
                fs.readFileSync(
                  path.join(contractsFolder, 'gemforge.deployments.json'),
                  'utf8',
                ),
              );
              const { address } = local.contracts.find(
                (a) => a.name === 'DiamondProxy',
              ).onChain;
