...
(async () => {
  try {
    const { $$, execCommands } =
      await require('./shared/bootstrap')('production');
    const { pushDockerImageToContainerRegistry, createDatabase } =
      await require('./shared/digitalocean');
            ...
            sub: {
              '(default)': {
                ...
                opts: [
                  [
                    '-r, --registry <id>',
                    'Docker container registry name',
                    'quickdapp',
                  ],
                  [
                    '-p --prefix <prefix>',
                ...
                action: async ({ prefix, registry }) => {
                  await pushDockerImageToContainerRegistry(
                    $$,
                    registry,
                    `${prefix}-all`,
                  );
