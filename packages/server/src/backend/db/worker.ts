...
export const inProgressOrPendingJobsQueryFilter = (
  extraCriteria?: any,
): any => ({
  finished: {
    equals: null,
...
export const scheduleJob = async <T extends object>(
  db: PrismaClient,
...
export const scheduleCronJob = async <T extends object>(
  db: PrismaClient,
  job: WorkerJobConfig<T>,
  cronSchedule: string,
) => {
  ...
  return await db.workerJob.create({
    data: {
      userId: job.userId,
      ...generateJobDates(
        parseCronExpression(cronSchedule).getNextDate(new Date()),
      ),
      type: job.type,
      data: sanitizeJobData(job.data),
      cronSchedule,
...
export const gerInProgressOrPendingJobOfTypeForUser = async (
  db: PrismaClient,
  userId: number,
  type: WorkerJobType,
) => {
...
export const rescheduleFailedJob = async (db: PrismaClient, job: WorkerJob) => {
  ...
  return db.workerJob.create({
    data: {
      ...generateJobDates(
        dateFrom(Date.now() + job.autoRescheduleOnFailureDelay),
        job.removeDelay,
      ),
      userId: job.userId,
      type: job.type,
...
export const rescheduleCronJob = async (db: PrismaClient, job: WorkerJob) => {
  ...
  return db.workerJob.create({
    data: {
      ...generateJobDates(
        parseCronExpression(job.cronSchedule!).getNextDate(new Date()),
        job.removeDelay,
      ),
      userId: job.userId,
      type: job.type,
