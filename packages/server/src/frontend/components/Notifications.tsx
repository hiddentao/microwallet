...
import {
  useGetMyUnreadNotificationsCount,
  useMarkAllNotificationsAsRead,
  useMarkNotificationAsRead,
  useMyNotifications,
} from '@/frontend/hooks';
import { dateFriendlyFormat } from '@uwallet/shared';
import { Notification } from '@uwallet/shared';
...
const Item: FC<{ data: Notification }> = ({ data }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={cn('w-full rounded-md bg-slate-500 relative', {
        'bg-slate-700': read,
        'text-sm': read,
        'cursor-pointer hover:outline hover:outline-1 hover:outline-slate-50':
          !read,
      })}
    >
      ...
      <div className="p-4 flex flex-row justify-between items-start">
        ...
        <div
          className={cn('text-right ml-4 w-20 text-gray-400', {
            'text-slate-500': read,
          })}
        >
...
export const NotificationsDialog: FC<PropsWithChildren<{}>> = ({
  children,
}) => {
  ...
  const onReachedBottomOfList = useCallback(
    (visible: boolean) => {
  ...
  return (
    <Dialog>
      <DialogTrigger aria-label="notifications">{children}</DialogTrigger>
      <DialogContent className="h-[80vh]" size="med">
        <DialogHeader>
          <DialogTitle className="font-heading mb-2">
            Notifications{' '}
            <Button size="xs" className="ml-4" onClick={markAllAsRead}>
              Mark all as read
            </Button>
          </DialogTitle>
        </DialogHeader>
        <div className="mt-8 mb-4">
          {notifications.data ? <List data={notifications.data} /> : null}
        </div>
        {notifications.isFetching ? <Loading className="mt-4" /> : null}
        {notifications.error ? (
          <ErrorMessageBox className="mt-4">{`${notifications.error}`}</ErrorMessageBox>
        ) : null}
    ...
    <NotificationsDialog>
      <div className="relative">
        {unreadNotifications.data ? (
          <PingAnimation className="absolute top-0 right-0" />
        ) : null}
        <Bell size={24} />
      </div>
    </NotificationsDialog>
