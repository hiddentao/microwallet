...
import { MyNotifications, PageParam } from '@uwallet/shared';
import { GetMyNotificationsQuery, Notification } from '@uwallet/shared';
import {
  InfiniteData,
  useInfiniteQuery,
  useQuery,
...
export const useMyNotifications = (initialPageParam: PageParam) => {
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      return request(graphqlApiEndpoint, getMyNotificationsQuery, {
        pageParam,
      });
    },
    initialPageParam,
    getNextPageParam: (lastPage, _, lastPageParam) => {
      ...
      if (lastPageParam.startIndex + lastPageParam.perPage >= r.total) {
        ...
        return {
          startIndex: ~~Math.min(
            r.startIndex + lastPageParam.perPage,
            r.total - 1,
          ),
...
export const useMarkNotificationAsRead = (id: number) => {
  ...
  return useMutation({
    mutationFn: async () => {
      return request(graphqlApiEndpoint, markNotificationAsReadMutation, {
        id,
      });
    },
    onMutate: async () => {
      ...
      ]) as InfiniteData<GetMyNotificationsQuery>;
